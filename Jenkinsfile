...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3561
        NAMESPACE = "dang-inc-project-uk"
        REGISTRY_HOSTNAME = "dangerousitishnik"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "dang-inst"
        DEPLOYMENT_NAME = "dang-inst-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'dang-inc-project-uk') {
